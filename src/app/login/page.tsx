import { signIn } from "@/lib/auth";

interface Props {
  searchParams: Promise<{ verify?: string; error?: string }>;
}

export default async function LoginPage({ searchParams }: Props) {
  const { verify, error } = await searchParams;

  return (
    <div className="min-h-screen bg-[#09090b] flex flex-col items-center justify-center px-4">

      {/* Logo */}
      <div className="mb-8 text-center">
        <div className="font-black text-3xl text-white tracking-tight mb-2">
          Azure<span className="text-sky-400">trade</span>hub
        </div>
        <div className="text-sm text-zinc-500">Panel privado de gestión</div>
      </div>

      <div className="w-full max-w-sm">
        <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-8">

          {verify ? (
            <div className="text-center">
              <div className="text-4xl mb-4">📬</div>
              <h2 className="text-xl font-bold text-white mb-2">Revisa tu email</h2>
              <p className="text-zinc-400 text-sm leading-relaxed">
                Te hemos enviado un enlace mágico para acceder. Válido durante 24 horas.
              </p>
            </div>
          ) : (
            <>
              <h2 className="text-xl font-bold text-white mb-1">Acceder</h2>
              <p className="text-sm text-zinc-500 mb-6">Elige cómo quieres entrar</p>

              {error && (
                <div className="bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-3 text-sm text-red-400 mb-4">
                  Error al iniciar sesión. Inténtalo de nuevo.
                </div>
              )}

              {/* Google */}
              <form action={async () => {
                "use server";
                await signIn("google", { redirectTo: "/dashboard" });
              }}>
                <button
                  type="submit"
                  className="w-full flex items-center justify-center gap-3 bg-white hover:bg-zinc-100 text-black font-semibold py-3 px-4 rounded-xl transition-all hover:scale-[1.02] active:scale-95 mb-3"
                >
                  <svg className="w-5 h-5" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                  </svg>
                  Continuar con Google
                </button>
              </form>

            </>
          )}
        </div>

        <p className="text-center text-xs text-zinc-700 mt-6">
          ¿No tienes cuenta?{" "}
          <a href="/" className="text-sky-400 hover:underline">
            Solicitar acceso →
          </a>
        </p>
      </div>
    </div>
  );
}

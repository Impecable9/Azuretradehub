    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-5 rounded-3xl p-7 flex flex-col justify-between min-h-[200px] relative overflow-hidden"
          ...
          <div>
            ...
            <div className="text-slate-400 text-sm mt-3">
              {thisMonth.length} pedidos cerrados · {totalAll.toFixed(0)} € total acumulado
            </div>
          </div>
        ...
        <div className="col-span-3 flex flex-col gap-4">
          ...
          <WhiteStat label="RFQs totales" value={String(rfqs.length)} sub={`${respondedRFQs.length} respondidos`} />
        </div>
        ...
        <div className="col-span-4 bg-white rounded-3xl shadow-sm border border-slate-100 p-6">
          <div className="flex items-start justify-between mb-4">
            ...
            {phoenixTotal > 0 && (
              <div className="text-right">
                <div className="text-[10px] text-slate-400 uppercase tracking-wide">Coste base</div>
          ...
          <div className="h-2 bg-slate-100 rounded-full mb-4 overflow-hidden">
            <div className="h-full rounded-full transition-all duration-500"
              style={{
                width: `${progressPct}%`,
                background: progressPct === 100 ? "#22c55e" : progressPct > 50 ? "#6366f1" : "#f59e0b",
              }} />
          </div>
          ...
          <div className="space-y-2">
            ...
            {componentStatus.length > 4 && (
              <div className="text-[11px] text-slate-400 pl-3.5">+{componentStatus.length - 4} más</div>
            )}
          </div>

          {phoenixQuote && (
            <Link href={`/quotes/${phoenixQuote.id}`}
              className="mt-4 flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-slate-900 transition-colors">
              <Package className="w-3.5 h-3.5" /> Ver presupuesto completo
            </Link>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-sm font-black text-slate-900 uppercase tracking-widest">Historial de presupuestos</h2>
        ...
        <div className="px-6 py-3 border-b border-slate-50 grid grid-cols-12 gap-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
          ...
          <span className="col-span-2">Ítems</span>
            ...
            {recentQuotes.map((q, idx) => {
              ...
              return (
                <Link key={q.id} href={`/quotes/${q.id}`}
                  className="grid grid-cols-12 gap-4 items-center bg-slate-50 hover:bg-slate-100 rounded-2xl px-4 py-3.5 transition-all duration-150 group">
                  <div className="col-span-1 text-xs font-black text-slate-300">#{String(quotes.length - idx).padStart(3, "0")}</div>
                  <div className="col-span-4">
                    <div className="font-bold text-slate-800 text-sm group-hover:text-slate-900 leading-tight">{q.title}</div>
                  ...
                  <div className="col-span-2 text-xs text-slate-500">
                    ...
                    {responded > 0 && <div className="text-green-600 font-semibold">✓ {responded} cotiz.</div>}
                  </div>
                  <div className="col-span-2 text-right font-black text-slate-900 tabular-nums text-sm">
